...
The question gives us a place to start. As we were reviewing the e-mails for [[#q4_question][Question 4]], a few intriguing ones stood out:
...
alabaster.snowball@northpolechristmastown.com via the [[#q4_question][EWA system]]. When
...
However, there's a slightly easier way. On the [[#q3_question][SMB FileStor]], we found
...
their DDE payloads, and execute them. It did this via =C:\Program Files\WindowsGrabber\alabaster_snowball.py=. That file also had credentials for the [[#q4_question][EWA system]]:
...
systems moving from the public IPs we found during the [[#recon][Recon stage]], to
private ones).
...
execution? We could use a number of techniques, such as DLL hijacking, but
...
injection. An import command in =alabaster_snowball.py=, such as:
